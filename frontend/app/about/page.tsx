...
───────────────────────────────────────────────────────────── */
const GLOBAL_CSS = `
  @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,700&family=DM+Sans:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;700&display=swap');
  ...
  @keyframes seatGlow  { 0%,100%{opacity:0.5} 50%{opacity:1} }
  @keyframes orbitA    { from{transform:rotateY(0deg)   translateZ(52px) rotateY(0deg)}    to{transform:rotateY(360deg)  translateZ(52px) rotateY(-360deg)} }
  @keyframes orbitB    { from{transform:rotateY(120deg) translateZ(52px) rotateY(-120deg)} to{transform:rotateY(480deg)  translateZ(52px) rotateY(-480deg)} }
  @keyframes orbitC    { from{transform:rotateY(240deg) translateZ(52px) rotateY(-240deg)} to{transform:rotateY(600deg)  translateZ(52px) rotateY(-600deg)} }
  ...
  *,*::before,*::after { box-sizing:border-box; }
  ...
  .val-card { position:relative; overflow:hidden; cursor:default; }
  .val-card::before {
    content:''; position:absolute; top:-60%; left:-80%;
    width:160%; height:200%;
    background:conic-gradient(from 0deg at 20% 30%, transparent 0deg, rgba(255,255,255,0.13) 15deg, transparent 30deg);
    transform:rotate(-20deg) scale(0.6); opacity:0;
    transition:opacity 0.45s, transform 0.6s cubic-bezier(0.16,1,0.3,1);
    pointer-events:none; z-index:2;
  }
  .val-card:hover::before { opacity:1; transform:rotate(-20deg) scale(1.1); }

  .icon-draw path,.icon-draw circle,.icon-draw polyline,.icon-draw line,.icon-draw polygon,.icon-draw rect {
    stroke-dasharray:200; stroke-dashoffset:200;
    transition:stroke-dashoffset 0.7s cubic-bezier(0.16,1,0.3,1);
  }
  .val-card:hover .icon-draw path,
  .val-card:hover .icon-draw circle,
  .val-card:hover .icon-draw polyline,
  .val-card:hover .icon-draw line,
  .val-card:hover .icon-draw polygon,
  .val-card:hover .icon-draw rect { stroke-dashoffset:0; }

  .term-cursor { display:inline-block; width:7px; height:13px; background:#FF6B00; animation:termBlink 1s step-end infinite; vertical-align:middle; margin-left:2px; border-radius:1px; }
`;

/* ─────────────────────────────────────────────────────────────
   TYPES
───────────────────────────────────────────────────────────── */
interface IconProps  { id: string; size?: number; color?: string; className?: string; }
interface RevealProps { children: ReactNode; delay?: number; from?: "bottom" | "left" | "right"; }
interface CountStatProps { to: number; label: string; color: string; icon: string; }
interface TiltCardProps  { children: ReactNode; intensity?: number; }
interface TimelineStepProps { step: number; title: string; desc: string; color: string; delay: number; isLast: boolean; }
interface SpotlightCardProps { icon: string; title: string; desc: string; color: string; bg: string; delay: number; number: string; }

/* ─────────────────────────────────────────────────────────────
   ICONS
───────────────────────────────────────────────────────────── */
function Icon({ id, size = 20, color = "currentColor", className = "" }: IconProps) {
  ...
  const icons: Record<string, React.ReactNode> = {
    eye:      <svg style={s} viewBox="0 0 24 24" className={className}><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" {...p}/><circle cx="12" cy="12" r="3" {...p}/></svg>,
    scale:    <svg style={s} viewBox="0 0 24 24" className={className}><path d="M12 3v18M5 6l-2 6h4L5 6zM19 6l-2 6h4L19 6z" {...p}/><path d="M3 18h6M15 18h6M8 21h8" {...p}/></svg>,
    lock:     <svg style={s} viewBox="0 0 24 24" className={className}><rect x="3" y="11" width="18" height="11" rx="2" ry="2" {...p}/><path d="M7 11V7a5 5 0 0 1 10 0v4" {...p}/></svg>,
    heart:    <svg style={s} viewBox="0 0 24 24" className={className}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" {...p}/></svg>,
    arrow:    <svg style={s} viewBox="0 0 24 24" className={className}><path d="M5 12h14M12 5l7 7-7 7" {...p}/></svg>,
    github:   <svg style={s} viewBox="0 0 24 24" fill={color} stroke="none" className={className}><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/></svg>,
    linkedin: <svg style={s} viewBox="0 0 24 24" fill={color} stroke="none" className={className}><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>,
    book:     <svg style={s} viewBox="0 0 24 24" className={className}><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20" {...p}/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z" {...p}/></svg>,
    globe:    <svg style={s} viewBox="0 0 24 24" className={className}><circle cx="12" cy="12" r="10" {...p}/><line x1="2" y1="12" x2="22" y2="12" {...p}/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" {...p}/></svg>,
    chart:    <svg style={s} viewBox="0 0 24 24" className={className}><rect x="3" y="12" width="4" height="9" {...p}/><rect x="10" y="7" width="4" height="14" {...p}/><rect x="17" y="3" width="4" height="18" {...p}/><line x1="2" y1="21" x2="22" y2="21" {...p}/></svg>,
    check:    <svg style={s} viewBox="0 0 24 24" className={className}><polyline points="20 6 9 17 4 12" {...p}/></svg>,
    users:    <svg style={s} viewBox="0 0 24 24" className={className}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" {...p}/><circle cx="9" cy="7" r="4" {...p}/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" {...p}/></svg>,
    map:      <svg style={s} viewBox="0 0 24 24" className={className}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" {...p}/><circle cx="12" cy="10" r="3" {...p}/></svg>,
    star:     <svg style={s} viewBox="0 0 24 24" className={className}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" {...p}/></svg>,
    zap:      <svg style={s} viewBox="0 0 24 24" className={className}><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" {...p}/></svg>,
  };
...
/* ─────────────────────────────────────────────────────────────
   COUNTER STAT
───────────────────────────────────────────────────────────── */
function CountStat({ to, label, color, icon }: CountStatProps) {
  ...
  return (
    <div ref={ref}
      style={{ textAlign: "center", padding: "36px 20px", borderRadius: 24, background: "white", border: `1.5px solid ${color}22`, boxShadow: `0 4px 24px ${color}0D`, position: "relative", overflow: "hidden", transition: "transform 0.3s" }}
...
/* ─────────────────────────────────────────────────────────────
   3D TILT CARD
───────────────────────────────────────────────────────────── */
function TiltCard({ children, intensity = 18 }: TiltCardProps) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={ref} onMouseMove={onMouseMove} onMouseLeave={onMouseLeave}
      style={{
        transform: `perspective(900px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg) scale(${isIdle ? 1 : 1.03})`,
        transition: isIdle ? "transform 0.7s cubic-bezier(0.23,1,0.32,1)" : "transform 0.08s linear",
        transformStyle: "preserve-3d", cursor: "default", position: "relative",
      }}>
      <div style={{ position: "absolute", inset: 0, borderRadius: 28, pointerEvents: "none", zIndex: 10, background: `radial-gradient(ellipse at ${tilt.gx}% ${tilt.gy}%, rgba(255,255,255,0.18) 0%, rgba(255,255,255,0.03) 55%, transparent 75%)`, mixBlendMode: "overlay", transition: isIdle ? "background 0.7s" : "background 0.06s" }} />
      {children}
    </div>
  );
}
...
/* ─────────────────────────────────────────────────────────────
   TIMELINE STEP
───────────────────────────────────────────────────────────── */
function TimelineStep({ step, title, desc, color, delay, isLast }: TimelineStepProps) {
  return (
...
/* ─────────────────────────────────────────────────────────────
   HERO RIGHT VISUAL — MP Score Card Stack
───────────────────────────────────────────────────────────── */
function HeroScoreVisual() {
  const [activeIdx, setActiveIdx] = useState(0);

  const mps = [
    { id: "MP #001", name: "Rajendra Singh",    state: "Rajasthan",      party: "BJP", attendance: 94, questions: 47, debates: 38, lci: 0.91, grade: "A", color: "#34D399" },
    { id: "MP #047", name: "Priya Venkatesh",   state: "Tamil Nadu",     party: "DMK", attendance: 88, questions: 39, debates: 31, lci: 0.87, grade: "A", color: "#60A5FA" },
    { id: "MP #112", name: "Arjun Mehta",       state: "Gujarat",        party: "BJP", attendance: 81, questions: 34, debates: 27, lci: 0.82, grade: "A", color: "#A78BFA" },
    { id: "MP #203", name: "Sunita Devi",       state: "Bihar",          party: "JDU", attendance: 76, questions: 28, debates: 22, lci: 0.74, grade: "B", color: "#FBBF24" },
    { id: "MP #318", name: "Mohammed Rafi",     state: "Kerala",         party: "INC", attendance: 61, questions: 18, debates: 14, lci: 0.61, grade: "C", color: "#FB923C" },
  ];

  useEffect(() => {
    const t = setInterval(() => setActiveIdx(i => (i + 1) % mps.length), 2400);
    return () => clearInterval(t);
  }, []);

  const mp = mps[activeIdx];
  ...
  return (
    <div style={{ width: "100%", maxWidth: 500 }}>
      {/* Main card */}
      <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: "28px", backdropFilter: "blur(12px)", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg,transparent,#FF6B00,#FFB347,transparent)", backgroundSize: "200%", animation: "gradShift 3s ease infinite" }} />

        {/* Header row */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 22 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 800, color: "rgba(255,107,0,0.85)", letterSpacing: "0.22em", textTransform: "uppercase", marginBottom: 4 }}>MP Performance Card</div>
            <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 20, fontWeight: 700, color: "white", transition: "all 0.4s" }}>{mp.name}</div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", marginTop: 2 }}>{mp.state} · {mp.party}</div>
          </div>
          {/* Grade badge */}
          <div style={{ textAlign: "center" }}>
            <div style={{ width: 56, height: 56, borderRadius: 16, background: `${gradeColor(mp.grade)}18`, border: `2px solid ${gradeColor(mp.grade)}50`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 32, fontWeight: 700, color: gradeColor(mp.grade), transition: "all 0.4s" }}>
              {mp.grade}
            </div>
            <div style={{ fontSize: 9, fontWeight: 800, color: "rgba(255,255,255,0.25)", marginTop: 4, letterSpacing: "0.1em" }}>GRADE</div>
          </div>
        </div>

        {/* 3 metric bars */}
        {[
          { label: "Attendance", val: mp.attendance, max: 100, color: "#34D399", suffix: "%" },
          { label: "Questions",  val: mp.questions,  max: 60,  color: "#60A5FA", suffix: "" },
          { label: "Debates",    val: mp.debates,    max: 50,  color: "#A78BFA", suffix: "" },
        ].map(m => (
          <div key={m.label} style={{ marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.45)", letterSpacing: "0.08em", textTransform: "uppercase" }}>{m.label}</span>
              <span style={{ fontSize: 12, fontWeight: 800, color: m.color, fontFamily: "'Cormorant Garamond',Georgia,serif" }}>{m.val}{m.suffix}</span>
            </div>
            <div style={{ height: 5, borderRadius: 3, background: "rgba(255,255,255,0.07)", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(m.val / m.max) * 100}%`, background: `linear-gradient(90deg,${m.color}70,${m.color})`, borderRadius: 3, transition: "width 0.8s cubic-bezier(0.16,1,0.3,1)" }} />
            </div>
          </div>
        ))}

        {/* LCI score */}
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 800, color: "rgba(255,107,0,0.7)", letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: 2 }}>LCI Score</div>
            <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 38, fontWeight: 700, color: mp.color, lineHeight: 1 }}>{mp.lci.toFixed(2)}</div>
          </div>
          {/* Dot nav */}
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            {mps.map((_, i) => (
              <button key={i} onClick={() => setActiveIdx(i)}
                style={{ width: i === activeIdx ? 20 : 7, height: 7, borderRadius: 4, border: "none", background: i === activeIdx ? "#FF6B00" : "rgba(255,255,255,0.15)", cursor: "pointer", transition: "all 0.3s", padding: 0 }} />
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginTop: 12 }}>
        {[
          { label: "A Grade MPs", val: "31%", color: "#34D399" },
          { label: "Avg LCI",     val: "0.62", color: "#FF6B00" },
          { label: "Sessions",    val: "12",   color: "#60A5FA" },
        ].map(s => (
          <div key={s.label} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "12px 14px", backdropFilter: "blur(8px)", textAlign: "center" }}>
            <div style={{ fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 22, fontWeight: 700, color: s.color, lineHeight: 1 }}>{s.val}</div>
            <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.3)", marginTop: 4, letterSpacing: "0.1em", textTransform: "uppercase" }}>{s.label}</div>
          </div>
        ))}
      </div>
    </div>
...
───────────────────────────────────────────────────────────── */
function SpotlightValueCard({ icon, title, desc, color, bg, delay, number }: SpotlightCardProps) {
  const [hovered, setHovered] = useState(false);
  const barRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (barRef.current) barRef.current.style.width = hovered ? "100%" : "0%";
  }, [hovered]);
  ...
  return (
    <Reveal delay={delay}>
      <div
        ...
        style={{
          borderRadius: 24,
          background: hovered ? `linear-gradient(135deg, ${color}F0 0%, ${color}CC 100%)` : "white",
          border: `1.5px solid ${hovered ? "transparent" : color + "20"}`,
          ...
          transition: "background 0.5s cubic-bezier(0.16,1,0.3,1), box-shadow 0.4s",
          boxShadow: hovered ? `0 28px 64px ${color}30` : "0 2px 12px rgba(0,0,0,0.05)",
        }}
      >
        {/* Ghost number */}
        <div style={{ position: "absolute", bottom: -20, right: 8, fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: 108, fontWeight: 900, color: hovered ? "rgba(255,255,255,0.07)" : `${color}08`, lineHeight: 1, userSelect: "none", pointerEvents: "none", transition: "color 0.5s, transform 0.5s", transform: hovered ? "scale(1.08) translateY(-8px)" : "scale(1)" }}>{number}</div>

        {hovered && <>
          <div style={{ position: "absolute", top: -50, right: -50, width: 180, height: 180, borderRadius: "50%", background: "rgba(255,255,255,0.07)", pointerEvents: "none", zIndex: 1 }} />
          <div style={{ position: "absolute", bottom: -30, left: -30, width: 120, height: 120, borderRadius: "50%", background: "rgba(255,255,255,0.05)", pointerEvents: "none", zIndex: 1 }} />
        </>}

        <div style={{ position: "relative", zIndex: 2 }}>
          <div style={{ width: 54, height: 54, borderRadius: 16, background: hovered ? "rgba(255,255,255,0.15)" : bg, border: `1.5px solid ${hovered ? "rgba(255,255,255,0.25)" : color + "25"}`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18, transition: "background 0.4s, border 0.4s" }}>
            <Icon id={icon} size={24} color={hovered ? "white" : color} className="icon-draw" />
          </div>
          <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 19, fontWeight: 700, color: hovered ? "white" : "#1A1A2E", marginBottom: 10, lineHeight: 1.2, transition: "color 0.4s" }}>{title}</div>
          <div style={{ fontSize: 13, color: hovered ? "rgba(255,255,255,0.85)" : "#9A8F83", lineHeight: 1.78, transition: "color 0.4s" }}>{desc}</div>
        </div>

        <div style={{ position: "relative", zIndex: 2 }}>
          <div style={{ height: 2, background: hovered ? "rgba(255,255,255,0.15)" : `${color}15`, borderRadius: 2, overflow: "hidden" }}>
            <div ref={barRef} style={{ height: "100%", width: "0%", background: hovered ? "rgba(255,255,255,0.6)" : color, borderRadius: 2, transition: "width 0.7s cubic-bezier(0.16,1,0.3,1)" }} />
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 10, fontSize: 11, fontWeight: 700, color: hovered ? "rgba(255,255,255,0.6)" : color, transition: "color 0.4s" }}>
            <span>{hovered ? "Core principle" : "Hover to reveal"}</span>
            <Icon id="arrow" size={11} color={hovered ? "rgba(255,255,255,0.6)" : color} />
          </div>
        </div>
      </div>
    </Reveal>
...
/* ─────────────────────────────────────────────────────────────
   BUILDER AVATAR — DNA orbit
───────────────────────────────────────────────────────────── */
function BuilderAvatar() {
  return (
    <div style={{ position: "relative", width: 110, height: 110, margin: "0 auto 24px" }}>
      <div style={{ position: "absolute", inset: -14, borderRadius: "50%", border: "1.5px solid rgba(255,107,0,0.18)", animation: "pulseRing 2.8s ease-in-out infinite" }} />
      <div style={{ position: "absolute", inset: -20, display: "flex", alignItems: "center", justifyContent: "center", transformStyle: "preserve-3d", perspective: "300px" }}>
        {([
          { color: "#FF6B00", anim: "orbitA 2.8s linear infinite" },
          { color: "#60A5FA", anim: "orbitB 2.8s linear infinite" },
          { color: "#A78BFA", anim: "orbitC 4.2s linear infinite" },
        ] as { color: string; anim: string }[]).map((dot, i) => (
          <div key={i} style={{ position: "absolute", width: 9, height: 9, borderRadius: "50%", background: dot.color, boxShadow: `0 0 10px ${dot.color}`, animation: dot.anim }} />
        ))}
      </div>
      <div style={{ position: "absolute", inset: -4, borderRadius: "50%", background: "conic-gradient(#FF6B00 0deg, #FFB347 90deg, rgba(255,255,255,0.04) 180deg, #FF6B00 360deg)", animation: "spinSlow 4s linear infinite" }} />
      ...
      <img
        ...
        onError={(e) => {
          ...
          const next = (e.currentTarget as HTMLImageElement).nextSibling as HTMLElement | null;
...
/* ─────────────────────────────────────────────────────────────
   TYPEWRITER HOOK
───────────────────────────────────────────────────────────── */
function useTypewriter(words: string[], speed = 90, deleteSpeed = 55, pause = 1800): string {
...
export default function AboutPage() {
  ...
  const typeWord = useTypewriter(["Witness", "Voice", "Mirror", "Reckoning", "Conscience"]);
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: "'DM Sans', system-ui, sans-serif", background: "#F7F2EA", color: "#1A1A2E", overflowX: "hidden" }}>
      ...
      <div style={{ background: "linear-gradient(150deg,#080f1e 0%,#0A1628 50%,#111827 100%)", position: "relative", overflow: "hidden", minHeight: "90vh", display: "flex", alignItems: "center" }}>
        ...
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "88px 48px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 60, alignItems: "center", position: "relative", zIndex: 1, width: "100%" }}>
          ...
          <div>
            ...
            <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(44px,5.5vw,76px)", fontWeight: 700, color: "white", lineHeight: 0.92, letterSpacing: "-2.5px", marginBottom: 26, ...fi(80) }}>
              Democracy<br />
              needs a<br />
            ...
            <p style={{ fontSize: 15, color: "rgba(255,255,255,0.5)", lineHeight: 1.9, maxWidth: 480, marginBottom: 0, ...fi(160) }}>
            ...
            <Reveal from="right" delay={80}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[
                  ...
                  <div key={m.label}
                    style={{ background: "#F9F6F0", borderRadius: 18, padding: 22, border: `1.5px solid ${m.color}18`, borderTop: `3px solid ${m.color}`, transition: "transform 0.3s" }}
      ...
      <section style={{ padding: "88px 48px", background: "white" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 18, marginTop: 40 }}>
            {[
              { icon: "eye",   title: "Radical Transparency", number: "01", desc: "Every metric is public, every formula is open-source. No black boxes, no hidden editorial decisions — just raw parliamentary data any citizen can verify.", color: "#FF6B00", bg: "#FEF3C7", delay: 0   },
              { icon: "scale", title: "Zero Political Bias",  number: "02", desc: "We measure attendance, questions, and debates — actions in the official record. No opinion is ever injected into the scoring algorithm. BJP or INC, the formula is identical.", color: "#2563EB", bg: "#DBEAFE", delay: 80  },
              { icon: "lock",  title: "Data Integrity",       number: "03", desc: "All data comes directly from PRS Legislative Research and official Lok Sabha records. We never modify, selectively present, or editorialize it — ever.", color: "#7C3AED", bg: "#EDE9FE", delay: 160 },
            ...
            ].map(v => <SpotlightValueCard key={v.title} {...v} />)}
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1240, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 88 }}>
          <div>
            ...
            {[
              ...
              { step: 3, title: "LCI Computation",   desc: "The Lok Sabha Civic Index (LCI) is a weighted composite: 40% attendance, 35% questions, 25% debate participation. Formula is open-source.", color: "#7C3AED" },
      ...
      <section style={{ padding: "88px 48px", background: "white", position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "start" }}>
            ...
            {/* Right: profile card + stat tiles */}
            <Reveal from="right" delay={80}>
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <TiltCard intensity={18}>
                  <div style={{ background: "linear-gradient(140deg,#0A1628 0%,#13243F 50%,#0F1E38 100%)", borderRadius: 28, padding: "36px 32px", boxShadow: "0 40px 100px rgba(10,22,40,0.35), 0 0 0 1px rgba(255,255,255,0.06)", position: "relative", overflow: "hidden" }}>
                    <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(rgba(255,255,255,0.03) 1px,transparent 1px)", backgroundSize: "20px 20px", pointerEvents: "none" }} />
                    ...
                    <div style={{ position: "relative", zIndex: 1 }}>
                      ...
                      <div style={{ textAlign: "center", marginBottom: 20 }}>
                        <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 26, fontWeight: 700, color: "white", marginBottom: 4 }}>Piyush Ladukar</div>
                        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", letterSpacing: "0.12em", textTransform: "uppercase" }}>Developer · Designer · Civic Tech</div>
                      </div>
                      ...
                      <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginBottom: 22 }}>
                        {["Next.js", "Flask", "Python", "TypeScript", "Open Data"].map(tag => (
                          <span key={tag} style={{ padding: "5px 14px", borderRadius: 100, background: "rgba(255,107,0,0.1)", border: "1px solid rgba(255,107,0,0.22)", fontSize: 11, color: "#FF9040", fontWeight: 600 }}>{tag}</span>
                        ))}
                      </div>
                      <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 14, padding: "14px 16px", borderLeft: "3px solid #FF6B00" }}>
      ...
      {/* ══════════════════════════════════════════════════════
          CTA
      ══════════════════════════════════════════════════════ */}
      <section style={{ padding: "110px 48px", background: "linear-gradient(270deg,#C94D00,#FF6B00,#FF9A3C,#E05500,#C94D00)", backgroundSize: "400% 400%", animation: "gradShift 7s ease infinite", position: "relative", overflow: "hidden", textAlign: "center" }}>
        ...
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontFamily: "'Cormorant Garamond',Georgia,serif", fontSize: "clamp(100px,18vw,240px)", fontWeight: 900, color: "transparent", WebkitTextStroke: "2px rgba(255,255,255,0.08)", whiteSpace: "nowrap", userSelect: "none", pointerEvents: "none" }}>INDIA</div>
        ...
        <Reveal>
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <p style={{ fontSize: 16, color: "rgba(255,255,255,0.72)", maxWidth: 440, margin: "0 auto 44px", lineHeight: 1.82 }}>
              Check how your MP is performing right now. Transparent, free, and built for 1.4 billion citizens.
            </p>
            <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
              <Link href="/ranking" style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "17px 38px", background: "#0A1628", color: "white", borderRadius: 100, textDecoration: "none", fontWeight: 700, fontSize: 15, boxShadow: "0 6px 28px rgba(10,22,40,0.4)", transition: "all 0.25s" }}
              ...
              <Link href="/analytics" style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "17px 38px", background: "rgba(255,255,255,0.15)", color: "white", border: "1.5px solid rgba(255,255,255,0.3)", borderRadius: 100, textDecoration: "none", fontWeight: 600, fontSize: 15, transition: "all 0.25s" }}
